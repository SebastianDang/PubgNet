    ...
    public class Client
    {
        ...
        public Client(string api_key)
        {
            if (string.IsNullOrEmpty(api_key))
                throw new ArgumentException("Client : Api Key cannot be empty.");
        ...
        public async Task<PubgNet.Model.Players.PlayerData> GetPlayersByUsernames(string[] playerNames)
        {
#if DEBUG_CLIENT
            ...
            string player_request = $"shards/{m_Platform}/players?filter[playerNames]={players}";
            string player_response = await m_HttpHandler.RequestAsync(player_request);
            var player_object = JsonConvert.DeserializeObject<PubgNet.Model.Players.PlayerData>(player_response);
            return player_object;
        }

        // Gets a list of players by id(s).
        // According to the api, this can only get data for up to 6 players.
        public async Task<PubgNet.Model.Players.PlayerData> GetPlayersByIds(string[] playerIds)
        {
#if DEBUG_CLIENT
            ...
            string player_request = $"shards/{m_Platform}/players?filter[playerIds]={players}";
            string player_response = await m_HttpHandler.RequestAsync(player_request);
            var player_object = JsonConvert.DeserializeObject<PubgNet.Model.Players.PlayerData>(player_response);
            return player_object;
        }
